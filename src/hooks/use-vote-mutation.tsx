...
const postVote = async (payload: VotePayload): Promise<VoteResponse> => {
    const response = await fetch('/api/votes', { // Target API endpoint from Part 7
        method: 'POST',
        headers: {
...
const deleteVote = async (payload: VotePayload): Promise<VoteResponse> => {
    const response = await fetch('/api/votes', { // Use the same endpoint with DELETE
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
        },
...
export function useVoteMutation() {
    ...
    const mutation = useMutation<VoteResponse, Error, { toolId: number, action: 'add' | 'remove' }>({
        ...
        onSuccess: (data, variables) => {
            toast.success(variables.action === 'add' ? 'Vote Cast!' : 'Vote Removed', {
                description: data.message || (variables.action === 'add' ? 'Thank you for your feedback.' : 'Your vote has been removed.'),
            });

            // Invalidate relevant queries to refetch data
            // 1. Invalidate the specific tool's details to update vote count/status
            queryClient.invalidateQueries({ queryKey: ['tool-details', variables.toolId] }); // Assuming toolId is part of the key
            // 2. Invalidate the user's vote status query
            queryClient.invalidateQueries({ queryKey: ['user-vote-status', variables.toolId] });
